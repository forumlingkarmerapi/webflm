...
<script type="text/javascript">
    ...
    $(function (){
        var table = $('#tabelku').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ url('dap/kategoripublikasi/getlist') }}",
            columns: [
            ...
            columnDefs: [
                { className: "ratatengah", "targets": [0,2]},
    ...
    function showFormadd(){
    	$('#modalku').modal('show').find('#modalku_content').load("{{ url('/dap/kategoripublikasi/add') }}");
	}
	function showFormedit(id){
		$('#modalku').modal('show').find('#modalku_content').load("{{ url('/dap/kategoripublikasi/edit') }}?id="+id);
	}
    ...
    function hapus(id){
		...
		swal({
			...
			if (willDelete) {
				$.ajax({
					url: "{{ url('/dap/kategoripublikasi/delete') }}",
					type: 'POST',
					data: {"id":id},
					dataType: 'json',
