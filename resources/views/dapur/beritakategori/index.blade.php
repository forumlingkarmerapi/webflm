...
<script type="text/javascript">
    ...
    $(function (){
        var table = $('#tabelku').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ url('dap/kategoriberita/getlist') }}",
            columns: [
                {data: 'DT_RowIndex', name: 'DT_RowIndex'},
                {data: 'kategori_berita', name: 'kategori_berita'},
                {data: 'kategori_berita_en', name: 'kategori_berita_en'},
                {data: 'kategori_slug', name: 'slug'},
                {data: 'action'}
            ],
            columnDefs: [
    ...
    function showFormadd(){
    	$('#modalku').modal('show').find('#modalku_content').load("{{ url('/dap/kategoriberita/add') }}");
	}
	function showFormedit(id){
		$('#modalku').modal('show').find('#modalku_content').load("{{ url('/dap/kategoriberita/edit') }}?id="+id);
	}
    ...
    function hapus(id){
		...
		swal({
			...
			if (willDelete) {
				$.ajax({
					url: "{{ url('/dap/kategoriberita/delete') }}",
					type: 'POST',
					data: {"id":id},
					dataType: 'json',
